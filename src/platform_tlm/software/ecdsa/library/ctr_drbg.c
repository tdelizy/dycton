                   ...
                   size_t entropy_len )
{
    ...
    if( ( ret = mbedtls_ctr_drbg_reseed( ctx, custom, len ) ) != 0 ){
        printf("mbedtls_ctr_drbg_seed_entropy_len ret = %d\n", ret);
        return( ret );
      }
...
int mbedtls_ctr_drbg_write_seed_file( mbedtls_ctr_drbg_context *ctx, const char *path )
{
    ...
    printf(">>>>>>mbedtls_ctr_drbg_write_seed_file\n");
    printf("addr = %u ", (unsigned int)path);
    unsigned int addr = (unsigned int)path;

    // if( ( f = fopen( path, "wb" ) ) == NULL )
    if( ( f = fmemopen( addr, MBEDTLS_CTR_DRBG_MAX_INPUT, "wb" ) ) == NULL )
...
int mbedtls_ctr_drbg_update_seed_file( mbedtls_ctr_drbg_context *ctx, const char *path )
{
    ...
    printf(">>>>>>mbedtls_ctr_drbg_update_seed_file\n");
    printf("addr = %u ", (unsigned int)path);
    // workaround to avoid including platform specific files
    // and for limitating impact on mbedtls code
    unsigned int addr = (unsigned int)path;



    // if( ( f = fopen( path, "rb" ) ) == NULL )
    if( ( f = fmemopen( (int*)addr, MBEDTLS_CTR_DRBG_MAX_INPUT, "rb" ) ) == NULL )
        return( MBEDTLS_ERR_CTR_DRBG_FILE_IO_ERROR );
