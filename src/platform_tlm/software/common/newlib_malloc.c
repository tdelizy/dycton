...
#define cALLOc		newlib_calloc_r
#define fREe		newlib_free_r
#define mALLOc		newlib_malloc_r
#define mEMALIGn	newlib_memalign_r
#define rEALLOc		newlib_realloc_r
#define vALLOc		newlib_valloc_r
#define pvALLOc		newlib_pvalloc_r
#define mALLINFo	newlib_mallinfo_r
#define mALLOPt		newlib_mallopt_r
...
// forcing external declaration for multi heap handling
extern Heap_ctx * alloc_context;


/* access macros */

#define bin_at(i)      ((mbinptr)((char*)&((alloc_context->bins)[2*(i) + 2]) - 2*SIZE_SZ))
...
#define sbrked_mem  (alloc_context->current_mallinfo.arena)
...
#endif
{
  ...
  assert((char*)p >= alloc_context->sbrk_base);
  if (p != top)
    assert((char*)p + sz <= (char*)top);
  else
    assert((char*)p + sz <= alloc_context->sbrk_base + sbrked_mem);
...
#endif
{
  ...
  if ((unsigned long)(mmapped_mem + sbrked_mem) > (unsigned long)alloc_context->max_total_mem)
    alloc_context->max_total_mem = mmapped_mem + sbrked_mem;
  return p;
}
...
#endif
{
  ...
  assert (chunk_is_mmapped(p));
  assert(! ((char*)p >= alloc_context->sbrk_base && (char*)p < alloc_context->sbrk_base + sbrked_mem));
  assert((n_mmaps > 0));
...
/*
  dycton simplified version : making assumption that sbrk will return consistent memory zone in terms of size and alignment
  asserting no front missalign and removing multithread behavior handling
  almost sbrk once at start requesting max possible heap size, next sbrk will generates an overflow
*/
#if __STD_C
...
#endif
{
  ...
  INTERNAL_SIZE_T    sbrk_size     = nb;//alloc_context->heap_max_size;
  ...
  if (alloc_context->sbrk_base == (char*)(-1))  /* First time through. Record base */
    alloc_context->sbrk_base = brk;
  ...
  if (front_misalign > 0)
  {
    print("malloc: front missalign in malloc_extend_top(): aborting.\n");
    exit(5);
  }

  if ((unsigned long)sbrked_mem > (unsigned long)alloc_context->max_sbrked_mem)
    alloc_context->max_sbrked_mem = sbrked_mem;

  if ((unsigned long)(sbrked_mem) > (unsigned long)alloc_context->max_total_mem)
    alloc_context->max_total_mem = sbrked_mem;
}
//---------------------------------
...
/* Dycton modification :
    - simplifications due to being platform specific (#ifdef removals)
    - sbrk calls in malloc_extend_top reclaim the whole bank size
    - no malloc trim call (each heap have a dedicated memory bank)
    - malloc early fail lazy implementation (see MALLOC_EARLY_FAIL_IMPLEM)
*/
...
#endif
{
    ...
    mbinptr   q;                       /* misc temp */
    int       idx_big;                 /* index for biggest free block computation */
...
#ifdef MALLOC_EARLY_FAIL_IMPLEM
    /* Early fail implementation for dycton project */
    if (nb+MINSIZE >= alloc_context->bfbs)
    {
        return 0;
    }
#endif
...
#ifdef MALLOC_EARLY_FAIL_IMPLEM
            // lazy bigest free block size monitoring (only computing it that way when we fail)
            alloc_context->bfbs = chunksize(top);
            idx_big = 127;
            while (last(bin_at(idx_big)) == bin_at(idx_big)){
                idx_big--;
                if (idx_big<64)
                    return 0;
            }
            if(alloc_context->bfbs <chunksize(first(bin_at(idx_big)))){
                alloc_context->bfbs = chunksize(first(bin_at(idx_big)));
            }
...
/* dycton adaptation :
    - deactivated trim for simplicity
    - early fail implementation : just checking if freed bloc isn't bigger that bigest free bloc size after consolidation
*/
...
#endif
{
    ...
    if (next == top)                            /* merge with top */
    {
        ...
        // dycton : experimental testing with only one process, no need for trim
        // if ((unsigned long)(sz) >= (unsigned long)alloc_context->trim_threshold)
        //   malloc_trim(RCALL alloc_context->top_pad);


#ifdef MALLOC_EARLY_FAIL_IMPLEM
        // biggest free block size tracking
        // Checking if new top is the biggest free block
        if (chunksize(top) > alloc_context->bfbs) {
            alloc_context->bfbs = chunksize(top);
        }
#endif
...
#ifdef MALLOC_EARLY_FAIL_IMPLEM
    if(alloc_context->bfbs <chunksize(p)){
        alloc_context->bfbs = chunksize(p);
    }
#endif
    MALLOC_UNLOCK;
  ...
  else
  {
    ...
    else
    {
      ...
      if (new_brk == (char*)(MORECORE_FAILURE)) /* sbrk failed? */
      {
        ...
        {
          sbrked_mem = current_brk - alloc_context->sbrk_base;
          set_head(top, top_size | PREV_INUSE);
        }
        check_chunk(top);
	MALLOC_UNLOCK;
...
// #endif
  ...
  alloc_context->current_mallinfo.ordblks = navail;
  alloc_context->current_mallinfo.uordblks = sbrked_mem - avail;
  alloc_context->current_mallinfo.fordblks = avail;
  alloc_context->current_mallinfo.keepcost = chunksize(top);
...
#endif
{
  ...
  local_max_total_mem = alloc_context->max_total_mem;
  local_sbrked_mem = sbrked_mem;
  local_mallinfo = alloc_context->current_mallinfo;
...
#endif
{
  ...
  MALLOC_LOCK;
  malloc_update_mallinfo();
  ret = alloc_context->current_mallinfo;
  MALLOC_UNLOCK;
  return ret;
}
...
#endif
{
  ...
  switch(param_number)
  {
    case M_TRIM_THRESHOLD:
      alloc_context->trim_threshold = value; MALLOC_UNLOCK; return 1;
    case M_TOP_PAD:
      alloc_context->top_pad = value; MALLOC_UNLOCK; return 1;
...
#if HAVE_MMAP
  #error "YOU DON'T HAVE MMAP DO YOU ?"
#endif
