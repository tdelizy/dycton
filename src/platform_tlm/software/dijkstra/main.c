...
#define NUM_NODES                           (100)//(500)
#define NONE                                (9999)

#define LARGE_RUN                           (100)
#define SMALL_RUN                           (20)
...
void _dijkstra(int chStart, int chEnd)
{
...
#ifdef DIJKSTRA_SMALL
    for (i = 0, j = NUM_NODES / 2; i < 20; i++, j++) {
#else
#ifdef DIJKSTRA_LARGE
    for (i = 0, j = NUM_NODES / 2; i < 100; i++, j++) {
#else
#error "you must define dijkstra run size (dijkstra.c)"
#endif
#endif
        j = j % NUM_NODES;
        _dijkstra(i, j);
