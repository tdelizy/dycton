...
#define DBG_DUMP(x) do {                                         \
    print("\n====================");print_dec(x);                            \
    print("\nc=");print_dec(c);                                 \
    print("\ncti=");print_dec(cti);                            \
    print ("\nnext=");print_dec(next);                            \
    print("\nprefix=");print_dec(prefix);print("\nter:");         \
    int g=0;for(g=0; g < 20; g++)                        \
    {print(" ");print_dec(terminators[g]);}print("\nnref:");       \
    int h=0;for(h=0; h < 20; h++)                        \
    {print(" ");print_dec(next_references[h]);}print("\nfref:");                 \
    int f=0;for(f=0; f < 265; f++)                        \
    {print(" ");print_dec(first_references[f]);}                \
    print("\n====================\n");                           \
} while (0)
...
#define WRITE_CODE(code,maxcode) do {                           \
    ...
    if ((code) < extras) {                                      \
        shifter |= ((long long)(code) << bits);                 \
        bits += code_bits;                                      \
    }                                                           \
    else {                                                      \
        shifter |= ((long long)(((code) + extras) >> 1) << bits);\
        bits += code_bits;                                      \
        shifter |= ((long long)(((code) + extras) & 1) << bits++);\
    }                                                           \
...
int lzw_compress (void (*dst)(int), int (*src)(void), int maxbits)
{
    ...
    uint32_t *first_references, *next_references;
    uint32_t *terminators;


    unsigned long long shifter = 0;
...
int lzw_decompress (void (*dst)(int), int (*src)(void))
{
    ...
    // short *prefixes;
    uint32_t *terminators, *reverse_buffer;
    uint32_t *prefixes;
    ...
    while (1) {
        ...
        do {
            ...
            shifter |= (long long) read_byte << bits;
     ...
     free (prefixes); free (reverse_buffer);free (terminators);
    return 0;
}
