...
/* DYCTON port by TDk@CITILab
 *
 * keeping modification as light as possible
 * but the Dycton platform doesn't support a filesystem
 * so in and out files will be stubed to memory zones
 * handled with fmemopen()
 *
 * command line arguments from the mediabench 2 oprofile run
 *
 *
 *
 *
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 10; i+=4) {
        printf("%x%x %x%x\n", *(char*)(H263_IN_ADDR+i), *(char*)(H263_IN_ADDR+i+1), *(char*)(H263_IN_ADDR+i+2), *(char*)(H263_IN_ADDR+i+3));
    }
...
#ifdef DYCTON_RUN
    streamfile = fmemopen((int*)H263_STREAM_ADDR, DATASET_SZ_STREAM_OUT, "wb");
#else
    ...
    /* Clear output files */
#ifdef DYCTON_RUN
    if ((cleared = fmemopen((int*)H263_OUT_ADDR, DATASET_SZ_OUT, "wb")) == NULL) {
#else
